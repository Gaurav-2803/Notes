...
FROM
    employee
WHERE
    first_name IN ('Jim', 'Michael', 'Johnny', 'David');
...
FROM
    employee
GROUP BY
    sex -- Find the total sales of each salesman
SELECT
    SUM(total_sales),
    emp_id
FROM
...
FROM
    client
WHERE
    client_name LIKE '%Highschool%';

/* Unions */
-- Find a list of employee and branch names
SELECT
    employee.first_name AS Employee_Branch_Names
FROM
    employee
UNION
SELECT
...
SELECT
    client.client_name AS Non - Employee_Entities,
...
--     END;
CREATE TABLE
    trigger_test (message VARCHAR(100));
...
CREATE TRIGGER
    my_trigger BEFORE
INSERT
    ON employee FOR EACH ROW BEGIN IF NEW.sex = 'M' THEN
INSERT INTO
    trigger_test
VALUES
    ('added male employee');
